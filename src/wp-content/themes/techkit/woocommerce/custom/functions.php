...
class WC_Functions {
	...
	public function __construct() {
		...
		add_action( 'loop_shop_per_page',                  array( $this, 'techkit_loop_shop_per_page' ), 20 );
		add_action( 'woocommerce_after_shop_loop',         array( $this, 'techkit_products_paginations' ), 10 );
		// Columns
		add_filter( 'loop_shop_columns',                    array( $this, 'loop_shop_columns' ) );
		add_filter( 'woocommerce_get_image_size_thumbnail', array( $this, 'techkit_product_imgae_size' ) );
		add_filter( 'woocommerce_get_image_size_single', array( $this, 'techkit_single_product_imgae_size' ) );
		add_filter( 'woocommerce_get_image_size_gallery_thumbnail', array( $this, 'techkit_product_thumbnail_imgae_size' ) );
		...
		if ( function_exists( 'YITH_WCWL_Frontend' ) && class_exists( 'YITH_WCWL_Ajax_Handler' )  ) {
			...
			add_action( 'wp_ajax_techkit_add_to_wishlist',                array( $this, 'add_to_wishlist' ) );
			add_action( 'wp_ajax_nopriv_techkit_add_to_wishlist',         array( $this, 'add_to_wishlist' ) );
		}
		...
		/* ====== Checkout Page ====== */
		add_filter( 'woocommerce_checkout_fields', array( $this, 'techkit_checkout_fields' ) );
	}
	...
	public function loop_shop_columns(){
		$cols = TechkitTheme::$options['products_cols_width'];
		return $cols;
	}

	public function techkit_product_imgae_size( $size ) {
		return array(
			'width' => 370,
			'height' => 450,
			'crop' => 1,
	...
	public function techkit_product_thumbnail_imgae_size( $size ) {
		return array(
			'width' => 155,
			'height' => 157,
			'crop' => 0,
	...
	public function techkit_single_product_imgae_size( $size ) {
		return array(
			'width' => 450,
			'height' => 515,
			'crop' => 1,
	...
	public function techkit_products_paginations(){
		TechkitTheme_Helper::pagination();
	}
	...
	public function single_product_layout_hooks() {
		...
		if( class_exists( Review::class )){
			add_action( 'woocommerce_single_product_summary', array( $this, 'radius_single_review_total_rating'), 5 );	
		}else {
	...
	public function single_socials_share(){
		...
		self::techkit_product_social_share();
		echo '</div>';
	}	
	...
	//Review rating
	public function radius_single_review_total_rating() {
		if( class_exists( Review::class )){
			...
			<div class="rtrs-rating-item">
		        <div class="rating-icon">
		            ...
		            <span><?php $total_rating = Review::getTotalRatings( get_the_ID() );
		            	echo $total_rating . esc_html__( ' Ratings', 'revieweb' );
		            ?></span>
		        </div>			        
			</div>
		<?php }
    ...
    public function techkit_loop_shop_per_page( $products ) {
        // Return the number of products you wanna show per page.
        $shop_posts_per_page = TechkitTheme::$options['products_per_page'];
	...
	public static function techkit_product_social_share(){
		...
		<div class="post-share-btn">
			<h5 class="item-label"><?php esc_html_e( 'Share:', 'techkit' );?></h5>
	...
	public static function print_quickview_icon( $icon = true, $text = false ){
		...
		if ( is_shop() && !TechkitTheme::$options['wc_shop_quickview_icon'] ) {
			return false;
		}

		if ( is_product() && !TechkitTheme::$options['wc_product_quickview_icon'] ) {
			return false;
		}
		...
		if ( $text ) {
			$html .= '<span>' . esc_html__( 'QuickView', 'techkit' ) . '</span>';
		}

		?>
		<a href="#" class="yith-wcqv-button" data-product_id="<?php echo esc_attr( $product->get_id() );?>" title="<?php esc_attr_e( 'QuickView', 'techkit' ); ?>"><?php echo wp_kses_post( $html ); ?></a>
		<?php
	}
	...
	public static function print_compare_icon( $icon = true, $text = true ){
		...
		if ( is_shop() && !TechkitTheme::$options['wc_shop_compare_icon'] ) {
			return false;
		}

		if ( is_product() && !TechkitTheme::$options['wc_product_compare_icon'] ) {
			return false;
		}
		...
		?>
		<a href="<?php echo esc_url( $url );?>" class="compare" data-product_id="<?php echo esc_attr( $id );?>" title="<?php esc_attr_e( 'Add To Compare', 'techkit' ); ?>"><?php echo wp_kses_post( $html ); ?></a>
		<?php
	}
	...
	public static function print_add_to_wishlist_icon( $icon = true, $text = false ){
		...
		if ( is_shop() && !TechkitTheme::$options['wc_shop_wishlist_icon'] ) {
			return false;
		}

		if ( is_product() && !TechkitTheme::$options['wc_product_wishlist_icon'] ) {
			return false;
		}
	...
	public static function get_stock_status() {
		global $product;
		return $product->is_in_stock() ? esc_html__( 'In Stock', 'techkit' ) : esc_html__( 'Out of Stock', 'techkit' );
	}

	// WooCommerce Checkout Fields Hook
    public function techkit_checkout_fields( $fields ) {
